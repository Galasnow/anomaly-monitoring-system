  ...
  methods: {
    ...
    async checkFolderExists() {
      try {
        const response = await axios.get(
          "http://localhost:3017/api/check-folder"
        );
    ...
    async runMainPythonScript() {
      try {
        ...
        const response = await axios.get("http://localhost:3017/api/run-main");
    ...
    checkFolderAndLoadFiles() {
      return new Promise((resolve, reject) => {
        ...
        const timeLimit = 10 * 60 * 1000; // 10分钟的时间限制 (单位：毫秒)
        ...
        const intervalId = setInterval(async () => {
          try {
            ...
            // 尝试获取文件夹中的文件
            const response = await axios.get(
              "http://localhost:3017/api/files_txt"
            );
    ...
    analyzeData() {
      // 先执行文件夹检查和 .tif 文件加载或 Python 脚本调用
