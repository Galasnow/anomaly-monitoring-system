...
async function onAnalyzeButtonClick() {
  try {
    // 1. 先检查文件夹是否存在
    const outTifFileUrl = "http://localhost:3017/api/files_Gaoxiong";
...
async function runMainPythonScript() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:3017/api/run_main_Gaoxiong"
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    const intervalId = setInterval(async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:3017/api/files_txt_Gaoxiong"
        );
...
async function onDayClickHandler(day) {
  ...
  if (selectedTiff) {
    const tiffUrl = `public/01_Taiwan_Port/01_Gaoxiong_Port/02_Output/${selectedTiff.fullName}`;
