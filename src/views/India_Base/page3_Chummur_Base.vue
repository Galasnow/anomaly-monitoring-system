...
export default {
  ...
  methods: {
    ...
    async checkFolderExists() {
      try {
        const response = await axios.get(
          "http://localhost:3017/api/check_folder_Chummur"
        );
      ...
      return new Promise((resolve, reject) => {
        ...
        const intervalId = setInterval(async () => {
          try {
            ...
            // 尝试获取文件夹中的文件
            const response = await axios.get(
              "http://localhost:3017/api/files_txt_Chummur"
            );
    ...
    async fetchTiffFiles() {
      try {
        const response = await axios.get(
          "http://localhost:3017/api/files_Chummur"
        );
    ...
    async onDayClickHandler(day) {
      ...
      const selectedTiff = this.tifFiles.filter(
