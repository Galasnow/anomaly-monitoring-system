  ...
  <div class="container">
    <div class="top-panel">
      ...
      <div class="time-selector-box">
        <h2 class="title">楚马要塞异常扩建动态监测</h2>
        <h3>开始日期</h3>
    ...
    <div v-if="isChartModalVisible" class="modal">
      <div class="modal-content">
        <h2 class="title">楚马要塞异常扩建动态监测曲线</h2>
  ...
  methods: {
    async initCesium() {
      ...
      this.viewer.camera.setView({
        destination: Cesium.Cartesian3.fromDegrees(78.55648, 32.66934, 900.0),
      });
    },
    ...
    async checkFolderExists() {
      try {
        const response = await axios.get('http://localhost:3017/api/check_folder_Chummur');
    ...
    async runMainPythonScript() {
      try {
        ...
        const response = await axios.get('http://localhost:3017/api/run_main_Chummur');
      ...
      return new Promise((resolve, reject) => {
        ...
        const intervalId = setInterval(async () => {
          try {
            ...
            // 尝试获取文件夹中的文件
            const response = await axios.get('http://localhost:3017/api/files_txt_Chummur');
    ...
    async fetchTiffFiles() {
      try {
        const response = await axios.get('http://localhost:3017/api/files_Chummur');
    ...
    async onDayClickHandler(day) {
      ...
      if (selectedTiff) {
        const tiffUrl = `public/02_India_Base/02_Chummur_Base/02_Output/${selectedTiff.fullName}`;  // 根据选择的完整文件名拼接 URL
    ...
    async loadTiffImage(tiffUrl) {
      try {
        ...
        const utmProjection = 'EPSG:32644'; //WGS_1984_Zone_44N
    ...
    initChart() {
      decode_CSV("public/02_India_Base/02_Chummur_Base/Chummur_Base_Area.csv")
        .then(csv_data => {
