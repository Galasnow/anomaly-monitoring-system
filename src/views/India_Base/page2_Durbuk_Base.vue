...
async function runMainPythonScript() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:3017/api/run_main_Durbuk"
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    const intervalId = setInterval(async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:3017/api/files_txt_Durbuk"
        );
...
async function onDayClickHandler(day) {
  ...
  if (selectedTiff) {
    const tiffUrl = `public/02_India_Base/01_Durbuk_Base/02_Output/${selectedTiff.fullName}`;
