    ...
    .then((AMap) => {
      ...
      // TODO:研究区-台海
      {
        AMap.plugin("AMap.DistrictSearch", function () {
          ...
          district.search("台湾", function (status, result) {
      ...
      var imageLayer = new AMap.ImageLayer({
        url: "src/views/testmap/aaa.png", //图片 Url
        bounds: new AMap.Bounds([68.9, 36.5], [70.9, 38.2]), //图片范围大小的经纬度，传入西南和东北的经纬度坐标
        zIndex: 180, //图层的层级
        zooms: [2, 20], //设置可见级别，[最小级别，最大级别]
      });
      map.add(imageLayer);

      var imageLayer0 = new AMap.ImageLayer({
        url: "src/views/testmap/bbb.png", //图片 Url
        bounds: new AMap.Bounds([68.9, 34], [70.9, 36]), //图片范围大小的经纬度，传入西南和东北的经纬度坐标
        zIndex: 180, //图层的层级
        zooms: [2, 20], //设置可见级别，[最小级别，最大级别]
      });
      map.add(imageLayer0);
      ...
      var imageLayer1 = new AMap.ImageLayer({
        url: "src/views/testmap/ccc.png", //图片 Url
        bounds: new AMap.Bounds([71.4, 36.5], [73.4, 38.2]), //图片范围大小的经纬度，传入西南和东北的经纬度坐标
        zIndex: 180, //图层的层级
        zooms: [2, 20], //设置可见级别，[最小级别，最大级别]
      });
      map.add(imageLayer1);
      ...
      var imageLayer = new AMap.ImageLayer({
        url: "src/views/testmap/ddd.png", //图片 Url
        bounds: new AMap.Bounds([71.4, 34], [73.4, 36]), //图片范围大小的经纬度，传入西南和东北的经纬度坐标
        zIndex: 180, //图层的层级
        zooms: [2, 20], //设置可见级别，[最小级别，最大级别]
      });
      map.add(imageLayer);
      ...
      var imageLayer = new AMap.ImageLayer({
        url: "src/views/testmap/fff.png", //图片 Url
        bounds: new AMap.Bounds([78, 34], [82, 38.5]), //图片范围大小的经纬度，传入西南和东北的经纬度坐标
        zIndex: 180, //图层的层级
        zooms: [2, 20], //设置可见级别，[最小级别，最大级别]
      });
      map.add(imageLayer);
      map.setZoomAndCenter(7.5, [75.4, 36.25]);
