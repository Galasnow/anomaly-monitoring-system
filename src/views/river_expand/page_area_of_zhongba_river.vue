...
onMounted(async () => {
  ...
  // 添加实体
  const river = cesium_viewer.entities.add({
    id: "river",
    position: Cesium.Cartesian3.fromDegrees(76.3575, 35.1824, 10),
    point: {
      pixelSize: 10,
  ...
  cesium_viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(76.3575, 35.1824, 60000),
    duration: 2,
  });
  ...
  handler.setInputAction((movement) => {
    ...
    if (Cesium.defined(pickedObject) && pickedObject.id?.id === "river") {
      router.push("page_indian_river_tributary");
    }
