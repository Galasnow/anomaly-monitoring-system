  ...
  <div class="container">
    <div class="top-panel">
      ...
      <div class="time-selector-box">
        <h2 class="title">哈萨纳巴德冰川堰塞湖监测时间</h2>
        <h3>开始日期</h3>
    ...
    <div v-if="isChartModalVisible" class="modal">
      <div class="modal-content">
        <h2 class="title">哈萨纳巴德冰川堰塞湖异常溃决动态监测曲线</h2>
  ...
  methods: {
    async initCesium() {
      ...
      this.viewer.camera.setView({
        destination: Cesium.Cartesian3.fromDegrees(74.55072, 36.358, 10000.0),
      });
    },
    ...
    async checkFolderExists() {
      try {
        const response = await axios.get('http://localhost:3017/api/check_folder_Hassanabad');
    ...
    async runMainPythonScript() {
      try {
        ...
        const response = await axios.get('http://localhost:3017/api/run_main_Bhatinda');
      ...
      return new Promise((resolve, reject) => {
        ...
        const intervalId = setInterval(async () => {
          try {
            ...
            // 尝试获取文件夹中的文件
            const response = await axios.get('http://localhost:3017/api/files_txt_Bhatinda');
    ...
    async fetchTiffFiles() {
      try {
        const response = await axios.get('http://localhost:3017/api/files_Hassanabad');
    ...
    async onDayClickHandler(day) {
      ...
      if (selectedTiff) {
        const tiffUrl = `public/04_Pakistan_Lake/01_Hassanabad/02_Output/${selectedTiff.fullName}`;  // 根据选择的完整文件名拼接 URL
    ...
    async loadTiffImage(tiffUrl) {
      try {
        ...
        console.log("Bounding box (WGS84):", bbox);
    ...
    initChart() {
      decode_CSV("public/04_Pakistan_Lake/01_Hassanabad/Hassanabad_Area.csv")
        .then(csv_data => {
          // 提取日期、面积（保留4位小数）和abnormal值
          const date_list = csv_data.map(item => item.date);
          const area_list = csv_data.map(item => parseFloat(item.area).toFixed(6)); // 保留4位小数
