...
const firstDate = ref("2022-04-01");
const secondDate = ref("2022-05-18");
...
const tiffRootPath = "/04_Pakistan_Lake/01_Hassanabad/02_Output";
const csvPath = "/04_Pakistan_Lake/01_Hassanabad/Hassanabad_Area.csv";
const tiffApiUrl = `${backendUrlPrefix}/files_Hassanabad`;
const mainScriptUrl = `${backendUrlPrefix}/api/run_main_Bhatinda`;
const finishResponseUrl = `${backendUrlPrefix}/files_txt_Bhatinda`;
  ...
  decode_CSV(csvPath)
    .then((csv_data) => {
      ...
      const area_list = csv_data.map((item) =>
        parseFloat(item.area).toFixed(6)
      );
