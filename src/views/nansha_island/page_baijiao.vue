...
async function initCesium() {
  ...
  viewer.value.camera.setView({
    destination: Cesium.Cartesian3.fromDegrees(113.3203, 8.1906, 100000),
  });
}
...
async function runMainPythonScript() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:3017/api/run-main_baijiao"
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    const intervalId = setInterval(async () => {
      try {
        ...
        // 尝试获取文件夹中的文件
        const response = await axios.get(
          "http://localhost:3017/api/baijiao_finish_txt"
        );
...
async function fetchTiffFiles() {
  try {
    ...
    tifFiles_baijiao.value = response.data.files.map((file) => ({
      fullName: file,
      shortName: file.substring(0, 8),
    }));

    mark_dates.value = [];
    const files = tifFiles_baijiao.value;
    // console.log(files)
    for (let i = 0; i < tifFiles_baijiao.value.length; i++) {
