...
onMounted(async () => {
  ...
  // 添加SK10实体
  const bai_jiao = cesium_viewer.entities.add({
    id: "bai_jiao",
    position: Cesium.Cartesian3.fromDegrees(113.3203, 8.1906, 10),
    point: {
      pixelSize: 10,
  ...
  const bisheng_jiao = cesium_viewer.entities.add({
    id: "bisheng_jiao",
    position: Cesium.Cartesian3.fromDegrees(113.6755, 8.9559, 10),
    point: {
      pixelSize: 10,
      color: Cesium.Color.RED,
      outlineColor: Cesium.Color.WHITE,
  ...
  cesium_viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(113.45, 8.5, 650000),
    duration: 2,
  });
  ...
  handler.setInputAction((movement) => {
    ...
    if (
      ...
      pickedObject.id?.id === "bisheng_jiao"
    ) {
      router.push("page_bishengjiao");
    }
