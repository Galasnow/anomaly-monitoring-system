...
async function onAnalyzeButtonClick() {
  try {
    // 1. 先检查文件夹是否存在
    const outTifFileUrl = "http://localhost:3017/api/files_Leh";
...
async function runMainPythonScript() {
  try {
    ...
    if (response.data.message === "main.py 执行已启动") {
      ...
      const finishResponseUrl = "http://localhost:3017/api/files_txt_Leh";
...
async function onDayClickHandler(day) {
  ...
  if (selectedTiff) {
    const tiffUrl = `public/03_India_Airport/04_Leh_Airport/02_Output/${selectedTiff.fullName}`;
