...
async function onAnalyzeButtonClick() {
  try {
    // 1. 先检查文件夹是否存在
    const outTifFileUrl = "http://localhost:3017/api/files_Shilong";
...
async function runMainPythonScript() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:3017/api/run_main_Shilong"
    );
    console.log("返回消息:", response.data.message);

    if (response.data.message === "main.py 执行已启动") {
      // 等待文件夹生成并检查是否有 finish.txt 文件
      const finishResponseUrl = "http://localhost:3017/api/files_txt_Shilong";
...
async function onDayClickHandler(day) {
  ...
  if (selectedTiff) {
    const tiffUrl = `public/03_India_Airport/07_Shilong_Airport/02_Output/${selectedTiff.fullName}`;
...
function initChart() {
  decode_CSV(
    "public/03_India_Airport/07_Shilong_Airport/Shilong_Airport_Area.csv"
  )
    .then((csv_data) => {
