...
async function onAnalyzeButtonClick() {
  try {
    // 1. 先检查文件夹是否存在
    const outTifFileUrl = "http://localhost:3017/api/files_Silchar";
...
async function runMainPythonScript() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:3017/api/run_main_Silchar"
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    const intervalId = setInterval(async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:3017/api/files_txt_Silchar"
        );
...
async function onDayClickHandler(day) {
  ...
  if (selectedTiff) {
    const tiffUrl = `public/03_India_Airport/02_Silchar_Airport/02_Output/${selectedTiff.fullName}`;
...
function initChart() {
  decode_CSV(
    "public/03_India_Airport/02_Silchar_Airport/Silchar_Airport_Area.csv"
  )
    .then((csv_data) => {
