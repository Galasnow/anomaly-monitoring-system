  ...
  <div class="container">
    ...
    <div v-if="isChartModalVisible" class="modal">
      <div class="modal-content">
        <h2 class="title">比夏纳空军基地异常扩建动态监测曲线</h2>
  ...
  methods: {
    async initCesium() {
      ...
      this.viewer.camera.setView({
        destination: Cesium.Cartesian3.fromDegrees(74.75462, 30.27766, 900.0),
      });
    },
    ...
    async checkFolderExists() {
      try {
        const response = await axios.get(
          "http://localhost:3017/api/check_folder_Bhatinda"
        );
    ...
    async fetchTiffFiles() {
      try {
        const response = await axios.get(
          "http://localhost:3017/api/files_Bhatinda"
        );
