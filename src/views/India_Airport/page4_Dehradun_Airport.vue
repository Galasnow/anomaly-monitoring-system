  ...
  <div class="container">
    ...
    <div v-if="isChartModalVisible" class="modal">
      <div class="modal-content">
        <h2 class="title">德拉敦机场异常扩建动态监测曲线</h2>
  ...
  methods: {
    async initCesium() {
      ...
      this.viewer.camera.setView({
        destination: Cesium.Cartesian3.fromDegrees(78.188906, 30.193841, 700.0),
      });
    },
    ...
    async checkFolderExists() {
      try {
        const response = await axios.get('http://localhost:3017/api/check_folder_Dehradun');
    ...
    async runMainPythonScript() {
      try {
        ...
        const response = await axios.get('http://localhost:3017/api/run_main_Dehradun');
      ...
      return new Promise((resolve, reject) => {
        ...
        const intervalId = setInterval(async () => {
          try {
            ...
            // 尝试获取文件夹中的文件
            const response = await axios.get('http://localhost:3017/api/files_txt_Dehradun');
    ...
    async fetchTiffFiles() {
      try {
        const response = await axios.get('http://localhost:3017/api/files_Dehradun');
    ...
    async onDayClickHandler(day) {
      ...
      if (selectedTiff) {
        const tiffUrl = `public/03_India_Airport/03_Dehradun_Airport/02_Output/${selectedTiff.fullName}`;  // 根据选择的完整文件名拼接 URL
    ...
    initChart() {
      decode_CSV("public/03_India_Airport/03_Dehradun_Airport/Dehradun_Airport_Area.csv")
        .then(csv_data => {
