  ...
  methods: {
    ...
    async checkFolderExists() {
      try {
        const response = await axios.get(
          "http://localhost:3017/api/check_folder_Dehradun"
        );
    ...
    async runMainPythonScript() {
      try {
        ...
        const response = await axios.get(
          "http://localhost:3017/api/run_main_Dehradun"
        );
    ...
    async fetchTiffFiles() {
      try {
        const response = await axios.get(
          "http://localhost:3017/api/files_Dehradun"
        );
    ...
    initChart() {
      decode_CSV(
        "public/03_India_Airport/03_Dehradun_Airport/Dehradun_Airport_Area.csv"
      )
        .then((csv_data) => {
