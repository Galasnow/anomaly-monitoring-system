  ...
  methods: {
    ...
    async checkFolderExists() {
      try {
        const response = await axios.get(
          "http://localhost:3017/api/check_folder_Lengpui"
        );
    ...
    async runMainPythonScript() {
      try {
        ...
        const response = await axios.get(
          "http://localhost:3017/api/run_main_Lengpui"
        );
      ...
      return new Promise((resolve, reject) => {
        ...
        const intervalId = setInterval(async () => {
          try {
            ...
            // 尝试获取文件夹中的文件
            const response = await axios.get(
              "http://localhost:3017/api/files_txt_Lengpui"
            );
    ...
    async fetchTiffFiles() {
      try {
        const response = await axios.get(
          "http://localhost:3017/api/files_Lengpui"
        );
    ...
    initChart() {
      decode_CSV(
        "public/03_India_Airport/05_Lengpui_Airport/Lengpui_Airport_Area.csv"
      )
        .then((csv_data) => {
