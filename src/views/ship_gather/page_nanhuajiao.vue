...
async function checkFolderExists() {
  try {
    const response = await axios.get(
      "http://localhost:3017/api/check-folder_nanhuajiao"
    );
...
async function onDayClickHandler(day) {
  ...
  if (selected) {
    const tiffUrl = `/Ship_Gather/02_Nanhuajiao/result/${selected.fullName}`;
    console.log(tiffUrl);
...
// 初始化图表
function initChart() {
  decode_CSV("/Ship_Gather/02_Nanhuajiao/02_nanhuajiao_Number.csv")
    .then((csv_data) => {
