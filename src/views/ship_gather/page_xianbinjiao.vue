  ...
  <div class="container">
    ...
    <div v-if="isChartModalVisible" class="modal">
      <div class="modal-content">
        <h2 class="title">仙宾礁船舶异常聚集监测曲线</h2>
...
async function initCesium() {
  ...
  viewer.value.camera.setView({
    destination: Cesium.Cartesian3.fromDegrees(116.5026, 9.7509, 100000),
  });
}
...
async function analyzeData() {
  try {
    // 1. 先检查文件夹是否存在
    const outTifFileUrl = "http://localhost:3017/api/files_xianbinjiao";
...
async function runMainPythonScript() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:3017/api/run-main_xianbinjiao"
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    const intervalId = setInterval(async () => {
      try {
        ...
        // 尝试获取文件夹中的文件
        const response = await axios.get(
          "http://localhost:3017/api/xianbinjiao_finish_txt"
        );
...
async function fetchTiffFiles() {
  try {
    const response = await axios.get(
      "http://localhost:3017/api/files_xianbinjiao"
    );
