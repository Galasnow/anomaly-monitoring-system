...
def generate_test_image_plots(wm_file_path, output_path, id_index):
    ...
    # 创建子图
    fig, axes = plt.subplots(nrows=4, ncols=5, figsize=(16, 13), sharex=True, sharey=True)
    ...
    for tif in latest_tif_list:
        if sub_plot_num < len(tif_list):
            ...
            axes[i][j].set_title(title, fontsize=16, fontweight='bold', pad=6)  # 标题 默认居中，pad=6是指标题距离图的位置
    ...
    # 计算比例尺的起始位置在 x 轴上的坐标
    lon0 = minx + (maxx - minx) * 0.4
    # 计算比例尺的起始位置在y轴上的坐标
    lat0 = miny + (maxy - miny) * 0.05
    # 添加比例尺
    add_scalebar(last_ax, lon0, lat0, length=2000)  # 假设比例尺长度为2000米
...
set_dir = main_dir + r"01Input/"
#set_dir = main_dir + r"test/merge/"
