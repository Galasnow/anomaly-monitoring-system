...
if __name__ == "__main__":
    ...
    # gmain_dir = r"../public/01_TaiWan_Port/01_Gaoxiong_Port/"
    ...
    # composed = transforms.Compose([RandomCrop(1024)])     #用128*128的矩形框进行随机裁剪
    file_path = './model/model3.pth'
    ...
    if True:
        for i in range(len(test_data)):
            sample = test_data[i]
            image_tensor = Variable(torch.FloatTensor(sample["image"]).cuda()).unsqueeze(0)
    ...
    result_merge_dir = main_dir + r"02_Output/"
    ...
    line_result = main_dir + r"Baijiao_Area.png"
