...
from modify_type import round_day, valid_box_on_2_images, draw_box_on_one_image
from predict_with_auto_crop import get_boxes_per_image, build_mask
# from auto_crop import batch_crop_image_by_grid
from summarize_platform_final import calculate_iou_score, write_csv, calculate_time_score, \
    calculate_size_score
from util.labels import number2yolo, arange_label, write_txt_label, remove_invalid_tensor_by_mask, \
...
if __name__ == "__main__":
    ...
    input_shp_path = f'./greater_zhanghua_windfarm/greater_zhanghua_windfarm.shp'
    ...
    model_path = os.path.join(current_path, f'./run/2025-08-03_12-28-02/best_model.pth')
    ...
    annotations_name_list = [f'{item.filename_stem}' for item in ori_list]
    ...
    if len(annotations_name_list) != len(existing_modified_label_list):
        ...
        for i in tqdm(range(1, max_platform_id + 1)):
            ...
            if round_day(acquire_time_i[-1] - acquire_time_i[0]) < 12 or p1 < 0.1 or calculate_size_score(platform_size) < 0.1:
                remove_ids.append(i)
    ...
    # geo_transform_list = [item['geo_transform'] for item in ori_list]
    geo_transform_list = [ori_list[0].geo_transform for _ in platform_coordinates]
    ##########
    ...
    platform_counts = np.zeros((len(ori_list)), dtype=np.intp)
    for i in range(len(ori_list)):
        ...
        for i, num in enumerate(arr):
            if num > 0:
                ...
                if n > 0 and num <= n + 4:
                    return i
        return -1
    anomaly_count_need_2d = [[i, i+1, i+2, i+3, i+4] for i in (range(5, 200, 5))]
    ...
    output_csv_file = f'{output_stem_path}/turbine_number.csv'
    ...
    for i, [id, lon, lat] in enumerate(platform_geo_location):
        ...
        feature.SetField('Score', float(calculate_time_score((time_table[i][1] - time_table[i][0]).days) * calculate_size_score(
    ...
    ship_counts = [np.sum(result_list[i][..., 0] == 1) if len(result_list[i]) != 0 else 0 
                   for i in range(len(ori_list))]
