...
def read_txt_label(path, dtype=np.float32):
    ...
    with open(path, 'r') as label_file:
        ...
        content = np.asarray(content, dtype=dtype)
    return content
...
def remove_invalid_tensor_by_mask(tensors: torch.Tensor, mask: np.ndarray):
    ...
    for i, tensor in enumerate(tensors):
        ...
        if not np.any(mask[y1:y2, x1:x2]):
            invalid_tensor_args.append(i)
...
def move_boxes(boxes, x_offset: int, y_offset: int):
    boxes[..., 0] += x_offset
    boxes[..., 1] += y_offset
    boxes[..., 2] += x_offset
    boxes[..., 3] += y_offset
