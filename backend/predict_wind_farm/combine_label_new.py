...
if __name__ == "__main__":
    ...
    for i, cluster in enumerate(tqdm(grid_list_cluster)):
        ...
        mask = build_mask(list(reversed(original_image_shape)), geo_transform, input_shp_path)
        ...
        if len(annotations_list) == 0:
            write_txt_label(f'{output_combine_label_path}/{ori_list[i].filename_stem}.txt',
                            [])
            continue
        all_annotations_list = torch.tensor(np.concatenate(annotations_list, axis=0))
        # logging.info(f'{all_annotations_list = }')
        ...
        ori_image_stem = f'{ori_list[i].filename_stem}'
        # ori_image_name = f'{ori_list[i].filename}'
        ...
        # logging.info(f'{info = }')
        write_txt_label(f'{output_combine_label_path}/{ori_image_stem}.txt', out_annotations_list)
