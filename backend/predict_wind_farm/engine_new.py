...
from config import NUM_CLASSES, EPOCHS, train_image_dir, train_anno_path, val_anno_path, val_image_dir, \
    NUMBER_OF_WORKERS, DEVICE, LR, save_dir, early_stop, BATCH_SIZE_TRAIN, BATCH_SIZE_VAL
from dataset_new import CocoDetection
from model import create_model, create_custom_faster_rcnn
...
def train_one_epoch(model, train_loader, val_loader, optimizer, current_epoch):
    ...
    # metric = coco_eval.stats[0]
    metric = coco_eval.stats[0] * 0.2 + coco_eval.stats[1] * 0.7 + coco_eval.stats[8] * 0.1
...
def test_result(model, data_loader, device, save_dir, max_test_images=8):
    model.eval()
    results = []
    save_folder = f'{save_dir}/test_result'
    ...
    with torch.no_grad():
        ...
        for images, targets in progress_bar:
            ...
            for i, output in enumerate(outputs):
                ...
                image = np.ascontiguousarray((images[i].cpu().numpy().copy()*255).astype(np.uint8).transpose((1,2,0)))
                # print(f'{image = }')
                # print(f'{image.shape = }')
                for box, score, label in zip(boxes, scores, labels):
                    result = {
                ...
                cv2.imwrite(f'{save_folder}/{image_id}.png', image)
    print(f'{results = }')
...
if __name__ == "__main__":
    ...
    for current_epoch in range(1, EPOCHS + 1):
        ...
        model, metric = train_one_epoch(model, train_loader, val_loader, optimizer, current_epoch)
        ...
        if metric > best_metric:
            ...
            if no_update_epoch >= early_stop:
                ...
                test_result(model, val_loader, DEVICE, new_save_dir)
                break
