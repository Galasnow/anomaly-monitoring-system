...
from config import input_shp_path
...
def calculate_iou(box1, box2):
    ...
    intersection = float(max(0, x2 - x1) * max(0, y2 - y1))
    if intersection == 0:
        return 0
...
def calculate_cious(bboxes1, bboxes2):
    import torch
    rows = bboxes1.shape[0]
    cols = bboxes2.shape[0]
...
def date_filter(file_list: Sequence[str], date_range: Sequence[datetime.date], pattern: str = r'1SDV_(.*)T(.*)T'):
    filtered_file_list = []
    for file in file_list:
        acquire_time_select = re.search(pattern, file)
        if acquire_time_select:
            acquire_time_str = str(acquire_time_select.group(1))
            acquire_time_str_formated = f'{acquire_time_str[0:4]}-{acquire_time_str[4:6]}-{acquire_time_str[6:8]}'
            acquire_time = datetime.datetime.strptime(acquire_time_str_formated, '%Y-%m-%d')
            if date_range[0] <= acquire_time <= date_range[1]:
...
@time_it
def get_acquire_time_list(filename_list: Sequence[str], pattern: str = r'1SDV_(.*)T(.*)_(.*)T'):
    acquire_time_list: List[datetime.date | None] = [None] * len(filename_list)
    for i, file in enumerate(filename_list):
        ...
        if acquire_time_select:
            ...
            acquire_time_str_formated = f'{acquire_time_str[0:4]}-{acquire_time_str[4:6]}-{acquire_time_str[6:8]}'
            acquire_time = datetime.datetime.strptime(acquire_time_str_formated, '%Y-%m-%d')
            acquire_time_list[i] = acquire_time
        else:
            warnings.warn(f'Acquire time not found for given pattern! Drop {file}')
    effective_file_list = [file_name for i, file_name in enumerate(filename_list) if acquire_time_list[i] is not None]
    acquire_time_list = [acquire_time for acquire_time in acquire_time_list if acquire_time is not None]
    return acquire_time_list, effective_file_list
...
def delete_non_max_items(original_array: np.ndarray, delete_column=(0, 1), score_column=-1):
    ...
    output: [[0, 2, 0.3]
             [1, 2, 0.4]]
    >>> input_example = np.asarray([[0, 1, 0.2], [1, 0, 0.1], [0, 2, 0.3], [1, 2, 0.4]])
    >>> output_example = delete_non_max_items(input_example)
    >>> output_example
    >>> [[0, 2, 0.3], [1, 2, 0.4]]
    """
...
def get_platform_ship_coordinates_by_day_new(result_list, ori_list):
    platform_coordinates_by_day = [[] for _ in range(len(result_list))]
    ship_coordinates_by_day = [[] for _ in range(len(result_list))]
    for i in range(len(result_list)):
        geo_transform = ori_list[i].geo_transform
...
def get_platform_ship_coordinates_by_day_final(result_list, original_image_list):
    platform_coordinates_by_day = [[] for _ in range(len(result_list))]
    ship_coordinates_by_day = [[] for _ in range(len(result_list))]
    for i in range(len(result_list)):
        ...
        for result in result_day_i:
            ...
            geo_x_max, geo_y_max = image_coordinates_2_latitude_longitude(geo_transform, (image_x_max, image_y_max))
            if obj_id != 0:
                platform_coordinates_by_day[i].append(
                    {'obj_type': 'platform',
                     'obj_id': obj_id,
                     'acquire_time': original_image_list[i].acquire_time,
                     ...
                     'geo_x_max': geo_x_max,
                     'geo_y_max': geo_y_max,
                     'image_center_x': (image_x_max + image_x_min) / 2.0,
                     'image_center_y': (image_y_max + image_y_min) / 2.0,
                     'geo_center_x': (image_x_max + image_x_min) / 2.0,
                     'geo_center_y': (image_y_max + image_y_min) / 2.0,
                     'image_shape_x': image_x_max - image_x_min,
                     'image_shape_y': image_y_max - image_y_min
                     }
                )
            else:
                ship_coordinates_by_day[i].append(
                    {'obj_type': 'ship',
                     'obj_id': obj_id,
                     'acquire_time': original_image_list[i].acquire_time,
                     'image_x_min': image_x_min,
            ...
            # logging.info(j)
            # logging.info(f'{platform_coordinates_by_day[i] = }')
            platform_coordinates_by_day[i].sort(key=lambda x: x['acquire_time'])
            # logging.info(platform_coordinates_by_day[i])
    return platform_coordinates_by_day, ship_coordinates_by_day
...
def get_platform_ship_coordinates_by_id(result_list, ori_list, max_platform_id):
    ...
    for i in range(max_platform_id):
        ...
        for record, geo_transform in zip(record_list, geo_transform_list):
            ...
            result.append(
                [image_x_max - image_x_min, image_y_max - image_y_min, geo_x_min, geo_x_max, geo_y_min, geo_y_max])
        result = np.asarray(result)
        average_x_size = np.mean(result[..., 0])
        average_y_size = np.mean(result[..., 1])
        average_x_geo = np.mean(result[..., 2] + result[..., 3]) / 2.0
        average_y_geo = np.mean(result[..., 4] + result[..., 5]) / 2.0
...
def get_platform_ship_coordinates_by_id_new(result_list, ori_list, max_platform_id):
    platform_coordinates_by_id = [{} for _ in range(max_platform_id)]
    full_acquire_time_list = [ori_list_item.acquire_time for ori_list_item in ori_list]
    full_geo_transform_list = [ori_list_item.geo_transform for ori_list_item in ori_list]
    for i in range(max_platform_id):
        ...
        # logging.info(f'{platform_coordinates_by_id[i] = }')
    return platform_coordinates_by_id


def get_platform_ship_coordinates_by_id_final(result_list, original_image_list, max_platform_id):
    platform_coordinates_by_id = [{} for _ in range(max_platform_id)]
    full_acquire_time_list = [ori_list_item.acquire_time for ori_list_item in original_image_list]
    full_geo_transform_list = [ori_list_item.geo_transform for ori_list_item in original_image_list]
    for i in range(max_platform_id):
        time_geo_transform_record_list = [[acquire_time, geo_transform, record]
                                          for acquire_time, geo_transform, record_day_i in
                                          zip(full_acquire_time_list, full_geo_transform_list, result_list)
        ...
        # print(f'{record_list = }')
        result = []
        for acquire_time, geo_transform, record in zip(acquire_time_list, geo_transform_list, record_list):
            image_x_min = record[1]
            image_y_min = record[2]
            image_x_max = record[3]
            image_y_max = record[4]
            ...
            result.append(
                [image_x_max - image_x_min, image_y_max - image_y_min, geo_x_min, geo_x_max, geo_y_min, geo_y_max])
        result = np.asarray(result)
        average_size_x = np.mean(result[..., 0])
        average_size_y = np.mean(result[..., 1])
        average_geo_x = np.mean(result[..., 2] + result[..., 3]) / 2.0
        average_geo_y = np.mean(result[..., 4] + result[..., 5]) / 2.0

        platform_coordinates_by_id[i] = {
            'obj_id': i + 1,
            'geo_center_x': average_geo_x,
            'geo_center_y': average_geo_y,
            'average_size_x': average_size_x,
            'average_size_y': average_size_y,
            'first_time': acquire_time_list[0],
            'last_time': acquire_time_list[-1],
            'time': (acquire_time_list[-1] - acquire_time_list[0]).days,
            'frequency': len(acquire_time_list),
        }
...
def check_contain_between_tif_and_shp(geo_transform, projection, shp_path):
    result = False
    driver = ogr.GetDriverByName('ESRI Shapefile')
    with driver.Open(shp_path) as file:
        layer = file.GetLayer()
        dir(layer)
        n = layer.GetFeatureCount()
        # logging.info(f'feature count = {n}')
...
def check_tif_contains_shape(tif_path, shp_path):
    # 打开TIFF文件并获取地理范围
    ...
    # 计算TIFF的地理范围 (min_x, max_x, min_y, max_y)
    tif_min_x = geo_transform[0]
    tif_max_x = geo_transform[0] + tif_width * geo_transform[1]
    tif_min_y = geo_transform[3] + tif_height * geo_transform[5]
    tif_max_y = geo_transform[3]

    # 打开Shapefile并获取总范围
    shp_ds = ogr.Open(shp_path)
    if shp_ds is None:
        raise ValueError("无法打开Shapefile")

    layer = shp_ds.GetLayer()
    shp_extent = layer.GetExtent()  # 返回 (min_x, max_x, min_y, max_y)

    # 比较范围
    tif_contains = (
            tif_min_x <= shp_extent[0] and  # TIFF最小经度 <= Shape最小经度
            tif_max_x >= shp_extent[1] and  # TIFF最大经度 >= Shape最大经度
            tif_min_y <= shp_extent[2] and  # TIFF最小纬度 <= Shape最小纬度
            tif_max_y >= shp_extent[3]  # TIFF最大纬度 >= Shape最大纬度
    )
...
if __name__ == "__main__":
    filename_list = [filename for filename in os.listdir(r'H:\zhongyan\gee_download_weizhou\S1_modified\VV')
                     if filename.endswith('.tif')]
    acquire_time_list, effective_file_list = get_acquire_time_list(filename_list, pattern=r'1SDV_(.*)T(.*)T')
    print(f'acquire_time_list = {acquire_time_list}')
    print(f'effective_file_list = {effective_file_list}')

    print(f'sorted_acquire_time_list = {sorted(acquire_time_list)}')
    print(f'sorted_effective_file_list = {sorted(effective_file_list)}')

    path = r'H:\zhongyan\gee_download_weizhou\S1_modified\VV'
    sentinel_1_list = [Sentinel1(path, filename) for filename in os.listdir(path)
                       if filename.endswith('.tif')]

    tif_path = r'H:\zhongyan\gee_download_weizhou\S1_modified\VV\S1A_IW_GRDH_1SDV_20180918T105743_20180918T105808_023752_029716_E273.tif'
    shp_path = r'H:\zhongyan\gee_download_weizhou\weizhou_oil_field_range_big\weizhou_oil_field.shp'
    print(check_tif_contains_shape(tif_path, shp_path))
    # shape_list = [item.shape for item in sentinel_1_list]
    # print(f'shape_list = {shape_list}')

    sorted_sentinel_1_list = build_sorted_sentinel_1_list(r'H:\zhongyan\gee_download_weizhou\S1_modified\VV')
    sorted_acquire_time_list = [item.acquire_time for item in sorted_sentinel_1_list]

    plt.scatter(sorted_acquire_time_list, np.ones_like(sorted_sentinel_1_list), marker='o', s=0.5)
    fig = plt.gcf()
