...
def create_image_annotation(file_path: Path, width: float, height: float, image_id: int):
...
def get_images_info_and_annotations(path, input_annotations_path, results=True, box2seg=False, combine_class=True):
    path = Path(path)
    annotations = []
    images_annotations = []
    if path.is_dir():
    ...
    for file_path in file_paths:
        ...
        label_file_name = f"{label_file_path}".replace('tif', 'txt')
        ...
        if annotations_path.exists(): # The image may not have any applicable annotation txt file.
            ...
            for line1 in label_read_line:
                ...
                float_x_center = float(w) * x_center
                float_y_center = float(h) * y_center
                float_width = float(w) * width
                float_height = float(h) * height

                min_x = round(float_x_center - float_width / 2.0, 1)
                min_y = round(float_y_center - float_height / 2.0, 1)
                width = round(float_width, 1)
                height = round(float_height, 1)
...
if __name__ == "__main__":
    root_path = r'H:\zhongyan\wind_farm\predict_wind_farm\datasets'
    image_path = f'{root_path}/images/train'
    label_path = f'{root_path}/labels/train'
    output_path = f'{root_path}/train.json'
    result = False
    (
        coco_format["images"],
        coco_format["annotations"],
    ...
    image_path = f'{root_path}/images/val'
    label_path = f'{root_path}/labels/val'
    output_path = f'{root_path}/val.json'
    result = False
    (
        coco_format["images"],
        coco_format["annotations"],
    ...
    image_path = f'{root_path}/images/test'
    label_path = f'{root_path}/labels/test'
    output_path = f'{root_path}/test.json'
    result = False
    (
        coco_format["images"],
        coco_format["annotations"],
