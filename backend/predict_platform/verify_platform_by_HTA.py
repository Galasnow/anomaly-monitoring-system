...
def sort_by_time(landsat_st_file_list):
    ...
    landsat_st_file_list = list(np.asarray(landsat_st_file_list, dtype = datetime.datetime)[sorted_index])
...
def calculate_high_temperature_anomaly(nir, near_swir, far_swir):
    """high temperature anomaly (HTA)"""
    hta = (far_swir - near_swir) / nir
    hta[hta < 0] = 0
    return hta
...
if __name__ == "__main__":
    ...
    input_sentinel_1c_path = 'D:/zhongyan/gee_download_wenlai/landsat/119057'
    ...
    for j in tqdm(platform_ids):
        ...
        output_path = f'{input_sentinel_1c_path}/output/'
        ...
        landsat_st_file_list = [file_name for file_name in os.listdir(input_sentinel_1c_path) if
        ...
        for i, st_file in enumerate(landsat_st_file_list):
            ...
            selected_s1_date = platform_i_s1_date_list[min(int(insert_place), len(platform_i_s1_date_list) - 1)]
            # logging.info(selected_s1_date)

            with gdal.Open(f'{input_sentinel_1c_path}/{st_file}') as file:
                image = file.ReadAsArray()
