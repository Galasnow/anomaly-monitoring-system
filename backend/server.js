...
const MAIN_PY_PATH =
    "D:/Web/01_TaiWan_Port/Code/main.py";
...
app.get("/api/sk10_platform_finish_txt", (req, res) => {
    try {
        ...
        let files = fs
            ...
            .map((f) => f) // 只返回文件名
            .sort((a, b) => {
                const timeA = fs.statSync(path.join(SK10_OUTPUT_PATH, a)).mtime.getTime();
                const timeB = fs.statSync(path.join(SK10_OUTPUT_PATH, b)).mtime.getTime();
                return timeB - timeA; // 按修改时间降序
            });
